...
void jacobi(double **phi, double **aux, double **f, int n, int iternum, MPI_Comm comm)
{
    ...
    for (int k = 0; k < iternum; k++)
    {
        ...
        if (rank == 0)
        {
            ...
            {
                aux[n - 1][j] =
                    (phi[n - 1][j - 1] + phi[n - 1][j + 1] + phi[n - 2][j] + rowbuf[j] + h2 * f[n - 1][j]) / 4.0;
            }
            // update right boundary
            for (int i = 1; i < n - 1; i++)
            {
                aux[i][n - 1] =
                    (phi[i + 1][n - 1] + phi[i - 1][n - 1] + phi[i][n - 2] + colbuf[i] + h2 * f[i][n - 1]) / 4.0;
            }
            // update right up corner
            aux[n - 1][n - 1] =
                (phi[n - 1][n - 2] + phi[n - 2][n - 1] + colbuf[n - 1] + rowbuf[n - 1] + h2 * f[n - 1][n - 1]) / 4.0;
